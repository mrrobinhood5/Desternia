tembed -color <color>
<drac2>
# TODO: move monsters out of the group once they are activated
...
me, settingsCVAR, returnstr, saveAbility  = character(), "ExploreSettings", "", ""
hasMonsters, isAllowed, isBar, validNextPosition, inCombat, hasTrap,  = False, True, False, False, False, False
            ...
            if not isBar and isAllowed:
                ...
                if inCombat and "trap" in loadedData[savedExploreSettings['setting']][position].keys():
                    ...
                    if trapSave.total < trapDC:
                        ...
                        damageRoll = False
                #this is where the monsters get reorganized
                if inCombat and "monsters" in loadedData[savedExploreSettings['setting']][position].keys():
                    hasMonsters = True
                    for monster, mname in loadedData[savedExploreSettings['setting']][position]["monsters"].items():
                        ...
                        #re names them
                        combat().get_combatant(monster).set_name(mname)
...
combat().get_combatant("explore").add_effect("settings",f"""-attack '||{savedExploreSettings}'""", -1, False, None, False,None)
